    ...
    <div className='lg:h-screen w-screen flex flex-col pt-2' id='home'>
        ...
        <div className={`flex lg:px-6 pt-10 sm:pt-20 lg:pt-10 lg:h-full xs:gap-10 md:gap-16 xs:mb-20 flex-grow flex-col-reverse justify-center items-center text-center lg:text-left lg:flex-row`}>
            <div className='flex flex-col lg:w-[50%] h-max relative xs:top-[5%] lg:top-[15%]  lg:left-20 space-y-4 xl:mb-40'>
                ...
                <h1 className='font-semibold text-[2.7rem]  font-quicksand bg-gradient-to-r from-[#7FF79E] to-[#24D2ED] text-transparent bg-clip-text'>
                    <Typewriter 
                    options={{
                ...
                <div className='flex gap-x-2 xs:justify-center lg:justify-start pt-2'>
                    <IconBrandLinkedin className={`hover:scale-110 cursor-pointer duration-100 ease-in-out ${themeMode?'hover:stroke-[#24D2ED]':'hover:stroke-amber-600'} `} color='#4c4c4c' size={27} stroke={2} onClick={()=>{window.open(linkedinUrl,'_blank')}}/>
                    <IconBrandGithub className={`hover:scale-110 cursor-pointer duration-100 ease-in-out ${themeMode?'hover:stroke-[#24D2ED]':'hover:stroke-amber-600'} `} color='#4c4c4c' size={27} stroke={2} onClick={()=>{window.open(githubUrl,'_blank')}}/>
                    {/* <IconBrandBehance className=className={`hover:scale-110 cursor-pointer duration-100 ease-in-out ${themeMode?'hover:stroke-[#24D2ED]':'hover:stroke-amber-600'} `}color='#4c4c4c' size={27} stroke={2}/> */}
                    <a href="mailto:fahadshah1060@gmail.com">
                    <IconMail className={`hover:scale-110 cursor-pointer duration-100 ease-in-out ${themeMode?'hover:stroke-[#24D2ED]':'hover:stroke-amber-600'} `} color='#4c4c4c' size={27} stroke={2}/></a>
                </div>
            ...
            <div className='flex justify-center lg:w-[50%]'>
                <div className='xs:w-[15rem] xs:h-[16rem] md:w-[21rem] md:h-[22rem] xl:mb-24 grayscale-0 hover:grayscale-0 duration-[3s] ease-in-out relative lg:top-[20%] lg:left-3 lef'>
                    <img className={`w-[100%] h-[100%] hero-image ${toggleImg?(themeMode?'border-[#1C507E]':'border-amber-500'):'border-gray-200'} object-cover border-8 ${themeMode?'md:hover:border-[#1C507E]':'hover:border-amber-500'} duration-[3s] ease-in-out`} src={HeroImg} alt="" onClick={handleImageToggle} />
                </div>
            </div>
